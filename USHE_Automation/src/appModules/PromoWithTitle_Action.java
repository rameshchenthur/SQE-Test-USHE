	...
	public static void PromoTitle(WebDriver driver) throws Exception{
		...
		for(int i=1;i<=ExcelUtils.getRowCount();i++)
		{
		   ...
		   if(driver.getTitle().contains("Create Promo"))
		       ...
		       String promo_Title=ExcelUtils.getCellData(i, 0);
		       Promo_OR.txtbx_existingTitle(driver).sendKeys(promo_Title);
		       
		       int Size=driver.findElements(By.xpath("//div[@id='autocomplete']/ul/li/div/div[contains(text(),'"+promo_Title+"')]")).size();
				  for(int j=1;j<=Size;j++)
		          {  
		    	      WebElement titleDropDown=driver.findElement(By.xpath("//div[@id='autocomplete']/ul/li["+j+"]/div/div"));
		    	      if(titleDropDown.getText().contains(promo_Title))
		    	         {
		    	    	      titleDropDown.click();
		    		          break;
		    	         }
